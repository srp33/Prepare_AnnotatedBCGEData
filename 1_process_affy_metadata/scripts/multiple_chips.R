...
for (gseID in dual_chips) {
    out_file_path_A <- file.path(metadata_dir, paste0(gseID, "_U133A.tsv"))

    if (file.exists(out_file_path_A)) {
      print(paste0(out_file_path_A, " already exists, so it will not be processed."))
    } else {
      df <- getDualGEO(gseID)
      ...
      if ((ncol(metadata_A) > 2)) {
        write_tsv(metadata_A, out_file_path_A)
...
out_file_path_A <- file.path(raw_metadata_dir, paste0("GSE6532_U133A.tsv"))
...
if (file.exists(out_file_path_A)) {
  ...
  # write un-curated metadata to file
  write_tsv(df_U133A, out_file_path_A)
